...
class JobSubmitter {
  ...
  throws ClassNotFoundException, InterruptedException, IOException {
    ...
    //validate the jobs output specs 
	  /**
	   * Step1:检查job的输出路径是否存在，如果存在则抛出异常。
	   */
    checkSpecs(job);
    
    /**
     * 初始化用于存放Job相关资源的路径。注意此路径的构造方式为：
     * conf.get(MRJobConfig.MR_AM_STAGING_DIR,MRJobConfig.DEFAULT_MR_AM_STAGING_DIR) + Path.SEPARATOR + user + Path.SEPARATOR + STAGING_CONSTANT
    ...
    /**
     * Step4:通过RPC，向Yarn的ResourceManager申请JobID对象。
     */
    ...
    try {
      ...
      /**
       * Step5:从HDFS的NameNode获取验证用的Token，并将其放入缓存
       */
      ...
      /**
       * Step6:将作业文件上传到HDFS，这里如果我们前面没有对Job命名的话，默认的名称就会在这里设置成jar的名字。
       * 并且，作业默认的副本数是10，如果属性mapreduce.client.submit.file.replication没有被设置的话。
       */
      ...
      /**
       * 对每个输入文件进行split划分。注意这只是个逻辑的划分，不是物理的。因为此处是输入文件，
       * 因此执行的是FileInputFormat类中的getSplits方法。只有非压缩的文件和几种特定压缩方式压缩后的文件才分片。
       * 分片的大小由如下几个参数决定：
       * mapreduce.input.fileinputformat.split.maxsize、
       * mapreduce.input.fileinputformat.split.minsize、文件的块大小。
       * 具体计算方式为：
       * Math.max(minSize, Math.min(maxSize, blockSize))
       * 分片的大小有可能比默认块大小64M要大，当然也有可能小于它，
       * 默认情况下分片大小为当前HDFS的块大小，64M。
       */
