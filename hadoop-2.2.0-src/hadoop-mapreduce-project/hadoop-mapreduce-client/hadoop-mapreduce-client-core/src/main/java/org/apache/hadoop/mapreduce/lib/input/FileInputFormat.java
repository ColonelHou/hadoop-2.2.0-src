...
public abstract class FileInputFormat<K, V> extends InputFormat<K, V> {
    ...
    for (FileStatus file: files) {
      ...
      if (length != 0) {
        ...
        if (isSplitable(job, path)) {
          ...
          //如果bytesRemaining超过分片大小splitSize一定量才会将文件分成多个InputSplit，SPLIT_SLOP(默认1.1)。
          while (((double) bytesRemaining)/splitSize > SPLIT_SLOP) {
            int blkIndex = getBlockIndex(blkLocations, length-bytesRemaining);
            //将符合条件的块的索引对应的block信息的主机节点以及文件的路径名、开始的偏移量、分片大小splitSize
            //封装到一个InputSplit中加入List<InputSplit> splits。
          ...
          //剩余如果bytesRemaining还不为0，表示还有未分配的数据，将剩余的数据及最后一个block加入splits。
          if (bytesRemaining != 0) {
            int blkIndex = getBlockIndex(blkLocations, length-bytesRemaining);
  ...
  /**
   * 接着就会执行如下方法获取block的索引，其中第二个参数是这个block在整个文件中的偏移量，
   * 在循环中会从0越来越大：
   * @param blkLocations
   * @param offset
   * @return
