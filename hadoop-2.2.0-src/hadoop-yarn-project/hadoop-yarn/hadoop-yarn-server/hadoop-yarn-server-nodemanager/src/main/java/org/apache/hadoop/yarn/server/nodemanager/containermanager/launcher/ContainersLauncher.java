 ...
 * 
 * 启动Container
 * 该过程主要工作是将运行container对应的完整shell命令写到私有目录下的launch_container.sh中，
 * 并将token文件写到container_tokens中。之所以要将container运行命令写到launch_container.sh中，
 * 然后通过运行shell脚本的形式运行container，主要是因为直接执行命令可能会有些特殊符号不识别。
 * 
 */
