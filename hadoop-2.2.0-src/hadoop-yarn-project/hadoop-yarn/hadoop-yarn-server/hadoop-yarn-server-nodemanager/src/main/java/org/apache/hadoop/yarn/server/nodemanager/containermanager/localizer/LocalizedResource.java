 ...
 * 
 * 是NodeManager中用于维护一种”资源”生命周期的数据结构，
 * 它维护了一个状态机，记录了“资源”可能存在的各个状态以及导致状态间转换的事件，
 * 当某个事件发生时，LocalizedResource会根据实际情况进行节点状态转移，同时触发一个行为。
 * 
 * 作用   是等待接收其他对象发出的ResourceEventType类型的事件，然后根据当前状态和事件类型，
 *     将当前状态转移到另外一种状态，同时触发另外一种行为（实际上执行一个函数，该函数可能会再次发出一种其他类型的事件）。
 * 
 */
