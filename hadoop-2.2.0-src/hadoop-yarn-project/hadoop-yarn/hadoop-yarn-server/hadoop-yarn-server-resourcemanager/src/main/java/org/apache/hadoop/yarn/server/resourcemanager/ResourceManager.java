 ...
 * 作业管理由ResourceManager实现
 * 组成 : 调度器（Scheduler|可插拔的组件） + 应用管理器（ApplicationsManager，ASM）
 *      调度器: 主要负责将集群中得资源分配给多个队列和应用,YARN自带了多个资源调度器，如Capacity Scheduler和Fair Scheduler等
 * 
