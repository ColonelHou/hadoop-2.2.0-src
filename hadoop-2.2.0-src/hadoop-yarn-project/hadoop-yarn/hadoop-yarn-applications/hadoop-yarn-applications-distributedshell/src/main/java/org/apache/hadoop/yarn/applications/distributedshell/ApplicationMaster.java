 ...
 * AM负责计算应用程序所需的资源量
 * 
 * 是每个用户作业的主进程,负责管理作业生命周期,包括动态增加和减少资源使用(即Container),管理执行流程
 * 
 * container是node manager创建的。RM需要告诉AM 以下信息， 跟哪个NM联系，
 * 申请的container ID，APP ID，资源量，安全信息。这些封装起来发给AM， AM拿着这些信息通过RPC找对应的NM，
 * NM按照对应信息在本地创建container，初始化，启动container,同时还要监控contianer,
 * 最终在初始化的工作空间里运行代码，把结果写HDFS中。这中间伴随着container状态的变化，
 * 会产生一系列事件，各种事件会驱动不同的服务完成相应的功能。创建系统进程时候的最终资源分配及限制，是操作系统的事情。
 * 
 * 功能:
 *     用户提交的每个应用程序均包含一个AM
 ...
 *     负责同调度器协商以获取合适的容器，并跟踪这些容器的状态和监控其进度
 * 
 * AM管理:
 * 	AMLivelinessMonitor
